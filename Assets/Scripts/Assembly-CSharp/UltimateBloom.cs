...
public class UltimateBloom : MonoBehaviour
{
	...
	public void CreateMaterials()
	{
		...
		string shaderPath = ((m_FlareType != FlareType.Single) ? "Hidden/Ultimate/FlareDouble" : "Hidden/Ultimate/FlareSingle");
	...
	{
		bool flag = false;
		flag = ((m_HDR != HDRBloomMode.Auto) ? (m_HDR == HDRBloomMode.On) : (source.format == RenderTextureFormat.ARGBHalf && GetComponent<Camera>().hdr));
			...
			if ((!(sources[num6] == null) || !m_DirectUpsample) && (array2[num6] || !m_DirectUpsample))
			{
				...
				if (!array2[num6])
				{
					if (renderTexture != null)
					{
						if (flag)
						{
							GaussianBlur1(renderTexture, renderTexture2, (m_AnamorphicDirection != AnamorphicDirection.Vertical) ? 0f : num7, (m_AnamorphicDirection != AnamorphicDirection.Horizontal) ? 0f : num8, null, BlurSampleCount.FourSimple, Color.white, 1f);
						}
						else
						{
				...
				else
				{
					...
					if (flag && renderTexture != null)
					{
						...
						GaussianBlur1(renderTexture, renderTexture3, (m_AnamorphicDirection != AnamorphicDirection.Vertical) ? 0f : num7, (m_AnamorphicDirection != AnamorphicDirection.Horizontal) ? 0f : num8, null, BlurSampleCount.FourSimple, Color.white, 1f);
